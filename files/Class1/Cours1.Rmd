...
library(kableExtra)
example <- read.csv("2021.csv")
example[c(1, 122, 3934, 20000), c("date_mutation", "code_postal", "valeur_fonciere", "lot1_surface_carrez")] %>%
  kbl() %>%
  kable_styling()
```
...
For this class we use house price data in 2021 in Paris you can find on `DVF`.
...
```{r}
immo <- read.csv("2021.csv", header = TRUE, sep = ",") 
```

--

Here, our data are stored in an `R` object called `immo`. You can give the name you want to your objects `r emo::ji("baby")`, but try to be consistent and kind with an external reader.
...
- Let's look at the first 3 rows of `immo`. (you can also do `head(immo)`)


```{r}
immo[1:3, 1:4] # show first 3 rows and 4 first columns
```

--

- To access one of the variable, you can use the `$` operator, for example with the city.

```{r}
immo$code_postal[1:3] # only the first 3
```

--

Note that this is equivalent as `immo[, "code_postal"]` or `immo[, 10]`.
...
Here we have at our disposal 33567 observations collected during 2021, with part of the columns being,
...
For example you can print the dimension of the dataframe stored in the object `immo`.
...
```{r}
summary(immo[, c("valeur_fonciere", "nature_mutation", "code_postal")]) 
```
...
```{r}
mean(immo$valeur_fonciere, na.rm = TRUE)
```
...
```{r}
mean(immo[immo$valeur_fonciere < 4000000 & immo$valeur_fonciere > 50000, "valeur_fonciere"], na.rm = TRUE)
```

--

Note that the option `na.rm = TRUE` is necessary because the data contain missing value. If not the output will be `NA`.
...
To ease readability you can do,

```{r, echo = FALSE}
mean(immo[immo$valeur_fonciere < 4000000 & immo$valeur_fonciere > 50000, 
          "valeur_fonciere"], 
     na.rm = TRUE)
```
...
```{r}
test <- immo$lot1_surface_carrez + immo$lot2_surface_carrez
test[95:103]
...
By the way, you can create a column composed by operation on other column such as `immo$my_new_column <- immo$lot1_surface_carrez + immo$lot2_surface_carrez`.
...
library(ggplot2)
ggplot(immo[immo$valeur_fonciere < 2000000 & immo$valeur_fonciere > 50000, ], aes(x = valeur_fonciere)) +  
...
ggplot(immo[immo$valeur_fonciere < 2000000 & immo$valeur_fonciere > 50000, ], aes(x = valeur_fonciere)) +  
  geom_histogram(bins = 50, # important parameter!
...
ggplot(immo, aes(x = nature_mutation)) +  
...
ggplot(immo[immo$valeur_fonciere < 10000000, ], aes(y = valeur_fonciere)) +  
...
ggplot(immo[immo$valeur_fonciere < 2000000 & immo$nombre_pieces_principales %in% c(1,2,3,4), ], aes(x = valeur_fonciere, y = as.factor(nombre_pieces_principales))) +
  geom_boxplot() + 
  theme_bw()
```
---
...
You can choose many other representations, for example violin plots...
...
and superposition of your real data points ! (in this specific plot it is a bit dirty...)


```{r, warning = F, fig.height = 4, fig.align = "center"}
ggplot(immo[immo$valeur_fonciere < 2000000 & immo$nombre_pieces_principales %in% c(1,2,3,4), ], aes(x = valeur_fonciere, y = as.factor(nombre_pieces_principales))) +
  geom_violin() + 
  geom_jitter(alpha = 0.2, width = 0.1, color = "darkblue")+
  theme_bw()
```

---
# Improve your code and reproducibility

For now we have been repeating at least one command. Which one?

--

Usually, at the beginning of your script or notebook, it is a good habits to note all your assumptions on data.
...
- keep in minn what you assumed
- change assumption in one click for the whole document

>Note that the convention is to use capital letters.

---
# Visualization: scatter plot

```{r, warning = F, fig.height =2 , fig.align = "center"}
ggplot(immo, aes(x = lot1_surface_carrez, y = valeur_fonciere)) +
  geom_point() + 
  theme_bw()
```
...
- Very popular metric
- Useful (but contains limits, don't forget it)
- The Pearson coefficient is the most used

--

The correlation coefficient between two variables $X_1$ and $X_2$ (for example `lot1_surface_carrez` and `valeur_fonciere`):
...
```{r}
cor(immo$lot1_surface_carrez, immo$valeur_fonciere, method = "pearson", use = "complete.obs")
```
...
```{r, warning = F, fig.height =2 , fig.align = "center", echo = FALSE, message = F}
ggplot(immo, aes(x = lot1_surface_carrez, y = valeur_fonciere)) +
  geom_point(alpha = 0.3, color = "pink") + 
  theme_bw() +
  geom_smooth(method = "lm") +
  ylim(0, VALEUR_FONCIERE_MAX) +
  xlim(0, SURFACE_MAX)
```

Does the linearity seems to be a good hypothesis for these data? Is it surprising?
...
Back to our `immo` example:

```{r}
res <- cor.test(immo$lot1_surface_carrez, immo$valeur_fonciere, method = "pearson", use = "complete.obs")
res
```
...
---
# Correlation only reflects linearity!
...
- In fact, this is only the common acceptation of the Simpson paradox. The initial paper proposes in fact three different explanations for this paradox.
...
```{r, warning = FALSE}
t.test(immo$valeur_fonciere, conf.level = 0.95)$conf.int
```


```{r, message = FALSE, warning = FALSE, echo = FALSE, fig.align="center", fig.height = 3, fig.width = 3}
ggplot(immo, aes(x = valeur_fonciere)) +
  geom_histogram(bins = 30) +
  geom_vline(xintercept = mean(immo$valeur_fonciere, na.rm = TRUE), color = "red")
```
...
For the 30 remaining minutes, two groups:
- for students already familiar with `R` an assignment with `immo` data,
- for the others, hands on and I'll help you.
---
...
- Download house price data on `https://www.data.gouv.fr/fr/datasets/demandes-de-valeurs-foncieres-geolocalisees/`
- Compute the evolution of mean price from 2016 to 2021 with confidence interval. 
>Useful commands are `group_by`, `summarise`, and `geom_errorbar`.

- If time, can you find an interesting visualization of what happened during spring 2020 on the house market?
