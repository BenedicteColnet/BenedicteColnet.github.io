...
$$\mathrm{R}^{2}=\frac{\|\hat{Y}\|^{2}}{\|Y\|^{2}}=\cos ^{2} \theta_{0}$$
...
--
You can also use `RegBest` to select the model.
...
```{r, echo = FALSE}
df1$is.female <- ifelse(df1$sex == "female", 1, 0)
```
...
- Prof. François Husson's book *R pour la statistique et la science des données* (`ozone` data analysis);

- Prof. Florian Oswald for their econometrics class at SciencePo (`ScPoEconometrics` for illustrations and labs).
...
If you want to dig into the theory you can read the book *Régression avec R* from Eric Matzner-Lober and Pierre-André Cornillon. You can also watch the Youtube series of Trevor Hastie and Robert Tishirani.
